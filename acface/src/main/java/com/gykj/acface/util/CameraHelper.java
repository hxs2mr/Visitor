...
//                    double rate = (double) supportedPreviewSizes.get(i).width / (double) supportedPreviewSizes.get(i).height;
//                    BigDecimal bd = new BigDecimal(rate);
//                    double result = bd.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
//                    if(result == 1.78){
//                        previewSize.width = supportedPreviewSizes.get(i).width;
//                        previewSize.height = supportedPreviewSizes.get(i).height;
//                        parameters.setPreviewSize(previewSize.width, previewSize.height);
//                        break;
//                    }else {
//                        if(i == supportedPreviewSizes.size()-1){
//                            cameraListener.onCameraError(new Exception("没有合适的是摄像头比例"));
//                        }
//                    }
//                }
    ...
    private int getCameraOri(int rotation) {
        ...
        if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            Log.d("hxs相机2",info.orientation+"   "+degrees);
